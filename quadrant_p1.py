...
st.set_page_config(page_title="回推平均營業額 × 成長率（互動）", layout="wide")
...
# Sidebar: upload only
# =========================
st.sidebar.header("資料來源（必須上傳）")
uploaded = st.sidebar.file_uploader("請上傳 Excel（.xlsx）", type=["xlsx"])

if uploaded is None:
    st.title("回推平均營業額 × 成長率（互動）")
    st.info("請先在左側上傳 Excel 檔案後，才會載入圖表與篩選器。")
    st.stop()
...
need_cols = [
    X_COL, Y_COL,
    BRAND_COL, ZONE_COL, CITY_COL, CITY_CODE_COL, CIRCLE_COL,
    MATCHED_COL, INCLUDE_COL
]
